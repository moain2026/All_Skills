...
**When audio supports narrative content** (product demos, brand films, explainers, voiced pieces), **never add:** equalizer bars, spectrum analyzers, waveform displays, musical notes clip art, generic particle systems, rainbow color cycling, strobing white on beats, abstract pulsing orbs. Instead, let content guide the visual and audio drive its behavior. Bass makes warmth _swell_. Treble sharpens _contrast_. The visual choice comes from "what does this piece feel like?"

**When music IS the content** (music visualizations, album visuals, concert visuals, audio-reactive art), spectrum analyzers, frequency bars, circular spectrums, and waveform displays are the correct visual vocabulary — they _are_ the content. See `effects.md § Audio Visualizer` for Canvas-based implementation patterns (bars, waveforms, circular layouts, multi-layer canvases).
