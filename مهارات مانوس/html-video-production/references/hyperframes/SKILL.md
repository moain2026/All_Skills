---
name: hyperframes
description: Create video compositions, animations, title cards, overlays, captions, voiceovers, audio-reactive visuals, and scene transitions in HyperFrames HTML. Use when asked to build any HTML-based video content, add captions or subtitles synced to audio, create audio-reactive animation (beat sync, glow, pulse driven by music), add animated text highlighting (marker sweeps, hand-drawn circles, burst lines, scribble, sketchout), or add transitions between scenes (crossfades, wipes, reveals, shader transitions). Covers composition authoring, timing, media, and the full video production workflow. For CLI commands (init, lint, preview, render) see the hyperframes-cli skill. In Manus, narration audio is generated via the native `generate_speech` tool — see ../_manus-overrides/media-generation.md.
---

<!-- Modified by Manus: description trimmed to remove `transcribe, tts` (those CLI subcommands are disabled in Manus); routing-table entries for tts.md and transcript-guide.md updated to point at Manus-native paths; narration.md entry annotated to clarify audio rendering goes through `generate_speech`. Original upstream: heygen-com/hyperframes@8d83d4f / skills/hyperframes/SKILL.md. See ../../_manus-overrides/modifications.md. -->
...
3. **NEVER use exit animations in standalone compositions** except on the final scene. If your entire video is in one `index.html`, the transition IS the exit. The outgoing scene's content MUST be fully visible at the moment the transition starts.
4. **Sub-compositions MUST handle their own exits.** In multi-file projects where each beat is a separate `.html` file loaded via `data-composition-src`, each sub-composition implements its own exit animation (e.g., `y:-150, blur:30px, 0.33s power2.in` for CSS transitions) and hard-kills its elements at the boundary with `tl.set(el, { opacity: 0, visibility: "hidden" })`. Shader transitions handle the blend automatically — no exit animation needed. Hard cuts need no exit animation.
5. **Final scene only:** The last scene may fade elements out (e.g., fade to black).
...
```bash
node references/hyperframes/scripts/animation-map.mjs <composition-dir> \
...
- **[references/tts.md](references/tts.md)** — Manus-native speech generation contract. Routes to `generate_speech` and `_manus-overrides/media-generation.md`. Read when generating narration or voiceover.
...
- **[references/narration.md](references/narration.md)** — Pacing, tone, script structure, number pronunciation, opening line patterns. Read when writing the spoken script. (Audio rendering of that script is done via `generate_speech` per [tts.md](references/tts.md).)
...
- **[references/transcript-guide.md](references/transcript-guide.md)** — Manus-native transcription routing: `generate_speech`-produced audio carries word-level timing already; only external user-supplied audio needs `manus-speech-to-text`. Read when caption work needs explicit per-word timestamps.
