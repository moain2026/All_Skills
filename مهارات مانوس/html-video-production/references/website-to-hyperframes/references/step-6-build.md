...
- **Shader transition**: no exit animation needed — the shader handles the blend.
- **Hard cut**: no exit animation. The scene simply ends.

For all CSS transition types and their GSAP implementations, read `references/hyperframes/references/transitions/catalog.md`.
...
**To verify choreography**, run the animation map:
